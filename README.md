...
In this example, we setup a `string` hello world WASM demo and demonstrate interaction with the SSVM addon. Although the `string` type is [NOT natively supported by WASM](https://medium.com/wasm/strings-in-webassembly-wasm-57a05c1ea333), our SSVM addon could interact with the wasm files generated by [ssvmup](https://github.com/second-state/ssvmup) and `wasm-bindgen`.
