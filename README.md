...
#### Use SSVM addon
After installing the SSVM addon, we could now interact with `rsa-example.wasm` generated by wasm32-wasi backend in Node.js.

- Create a new folder at any path you want. (e.g. `mkdir application`)
- Copy `rsa-example.wasm` into your application directory. (e.g. `cp hello_gb.wasm <path_to_your_application_folder>`)
