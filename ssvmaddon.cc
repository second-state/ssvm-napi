...
Napi::Value SSVMAddon::RunStart(const Napi::CallbackInfo &Info) {
  ...
  if (!Result) {
    ThrowNapiError(ErrorType::ExecutionFailed);
    return Napi::Value();
  }
...
Napi::Value SSVMAddon::RunIntImpl(const Napi::CallbackInfo &Info,
  ...
  if (Res) {
    ...
    case IntKind::UInt64:
      ...
      [[fallthrough]];
    default:
      ThrowNapiError(ErrorType::NAPIUnkownIntType);
      return Napi::Value();
    }
  } else {
    ThrowNapiError(ErrorType::ExecutionFailed);
...
Napi::Value SSVMAddon::RunString(const Napi::CallbackInfo &Info) {
  ...
  if (auto ResultMem = MemInst->getBytes(ResultMemAddr, 8)) {
    ...
    ThrowNapiError(ErrorType::BadMemoryAccess);
    return Napi::Value();
  }
...
void SSVMAddon::LoadWasm(const Napi::CallbackInfo &Info) {
  ...
  if (BC.isFile() && !VM->loadWasm(BC.getPath())) {
    ThrowNapiError(ErrorType::LoadWasmFailed);
    return;
  ...
  if (!(VM->validate())) {
    ThrowNapiError(ErrorType::ValidateWasmFailed);
    return;
  }

  if (!(VM->instantiate())) {
    ThrowNapiError(ErrorType::InstantiateWasmFailed);
    return;
  }
